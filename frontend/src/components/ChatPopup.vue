  ...
  <div class="chat-popup" :style="{ right: position.right + 'px', bottom: position.bottom + 'px' }">
    ...
    <div class="popup-body" ref="messagesContainer">
      ...
      <div v-else class="messages-list">
        <div v-for="msg in messages" :key="msg.id" :class="['message', msg.is_from_me ? 'mine' : 'theirs']">
    ...
    <div class="popup-footer">
      <form @submit.prevent="sendMessage">
        <input v-model="newMessage" @input="onInput" placeholder="Type a message..." class="chat-input">
...
.status { font-size: 0.7rem; color: #888; text-transform: capitalize; }
.status.online { color: #00ff00; }
.status.in-game { color: #ff7eb3; }
...
.popup-body::-webkit-scrollbar-thumb { background: #444; border-radius: 2px; }
...
.typing { font-size: 0.7rem; color: #888; font-style: italic; margin-left: 10px; }
.loading, .empty { text-align: center; color: #666; margin-top: 20px; font-size: 0.8rem; }
...
.popup-footer form { display: flex; gap: 8px; }
...
.chat-input:focus { border-color: #7afcff; }
